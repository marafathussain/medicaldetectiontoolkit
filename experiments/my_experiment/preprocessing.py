...
def pp_patient(inputs):
    ...
    ix, path = inputs
    pid = path.split('/')[-1]
    img = sitk.ReadImage(os.path.join(path, 'imaging.nii.gz'))
    ...
    img_arr = np.clip(img_arr, -200, 350)
    ...
    seg = sitk.ReadImage(os.path.join(path, 'segmentation.nii.gz'))
    seg_arr = sitk.GetArrayFromImage(seg)
    seg_arr[seg_arr == 2] = 0
    seg_arr = resample_array(seg_arr, img.GetSpacing(), cf.target_spacing)
...
if __name__ == "__main__":
    ...
    pp_dir = '/home/marafath/scratch/kits/train'
    ...
    subprocess.call('cp {} {}'.format(os.path.join(pp_dir, 'info_df.pickle'), os.path.join(pp_dir, 'info_df_bk.pickle')), shell=True)